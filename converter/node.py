...
class Node:
  ...
  def __init__(self, node_type: str, name: str, inputs: List[str],
               outputs: List[str], attrs=None, consts=None):
    """Initialize.
  ...
  def inference_shape(self, name_to_shape, name_to_node):
    if self.name in name_to_shape:
        return
    output_chunk = len(self.output_indexes)
    output_shape = [output_chunk, self.output_dim]
    name_to_shape[self.name] = output_shape
...
def make_node(node_type: str, name: str, inputs: List[str], outputs: List[str],
  ...
  if node_type == KaldiOpType.Gemm.name:
    return GemmNode(node_type, name, inputs, outputs, attrs, consts)
    ...
    return IdentityNode(node_type, name, inputs, outputs, attrs, consts)
    ...
    return Node(node_type, name, inputs, outputs, attrs, consts)
